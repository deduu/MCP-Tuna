...
class FineTuningService:
    ...
    def list_available_models(self, directory: str = "./") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for item in path.iterdir():
                if item.is_dir():
                    ...
                    if (item / "adapter_config.json").exists() or \
                       (item / "config.json").exists() or \
                       (item / "pytorch_model.bin").exists():
                        models.append({
                            "name": item.name,
                            "path": str(item),
                            "is_adapter": (item / "adapter_config.json").exists()
                        })
    ...
    async def check_local_cache(self) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for repo in cache_info.repos:
                repos.append({
                    ...
                    "last_accessed": str(repo.last_accessed) if hasattr(repo, "last_accessed") else None,
                    "last_modified": str(repo.last_modified) if hasattr(repo, "last_modified") else None,
                })
            
            return {
                "success": True,
                "cache_dir": str(cache_info.cache_dir),

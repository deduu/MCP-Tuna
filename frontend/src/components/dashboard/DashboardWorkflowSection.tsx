...
export function DashboardWorkflowSection({ cardless = false }: DashboardWorkflowSectionProps) {
  const navigate = useNavigate()
  const workflowGroups = WORKFLOW_GROUPS.map((group) => (
    ...
    return (
      <section className="space-y-4">
        <div className="space-y-1">
          <h3 className="text-base font-semibold">Start A Workflow</h3>
  ...
  return (
    <Card className="border-border/70">
      ...
      <CardContent className="space-y-4">{workflowGroups}</CardContent>
    </Card>
  )
}

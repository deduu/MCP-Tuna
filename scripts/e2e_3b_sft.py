"""
MCP Tuna E2E — Regular SFT: Llama 3.2 3B on 4 GB VRAM
========================================================
...
OUTPUT_DIR = os.path.join(AGENTY_ROOT, "output", "llama-3.2-3B-skk-sft")
RESULTS_DIR = os.path.join(AGENTY_ROOT, "output", "3b_sft_eval_results")
...
async def main():
    ...
    print("=" * 70)
    print("  MCP Tuna E2E: 3B Regular SFT + Two-Step Evaluation")
    print("=" * 70)
    ...
    # ── STEP 1: Regular SFT (no curriculum sorting) ──
    print("\n" + "-" * 70)
    print("  [3/6] Regular SFT (3B, 4-bit QLoRA)")
    print("-" * 70)

    n_train = sum(1 for _ in open(TRAIN_PATH, encoding="utf-8"))
    print(f"  Model:                  {BASE_MODEL}")
    print(f"  Training data:          {n_train} examples (original order)")
    print("  Epochs:                 1")
    print("  max_seq_length:         256  (conservative for 4 GB VRAM)")
    ...
    print("  TIMING")
    print("=" * 70)
    print(f"  Training (SFT):         {fmt_time(train_time)}")
